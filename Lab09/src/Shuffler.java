...
public class Shuffler {
	...
	public static int[] perfectShuffle(int[] values) {
		...
		for(int i = 0; i < values.length; i++){
			if(i < values.length/2){
				...
				secondHalf[i - values.length/2] = values[i];
			}
		}
	...
	public static int[] selectionShuffle(int[] values) {
		...
		int[] deck = new int[values.length];
		List<Integer> unshuffled = new ArrayList<>();
		
		for(int i =0; i < values.length; i++){
			unshuffled.add(values[i]);
		}
		
		for(int i =0; i < deck.length; i++){
			int toRemove = ran.nextInt(unshuffled.size());
			deck[i] = unshuffled.get(toRemove);
			unshuffled.remove(toRemove);
		}
